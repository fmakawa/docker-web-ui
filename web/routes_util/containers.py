...
@app.route('/containers/inspect/<container>')
def inspect(container):
    config = docker_client().inspect_container(container)
    inspect_data=json.dumps(config)
    return render_template(
        "containers/after_commands/after_inspect.html",
        inspect=config,
        inspect_data=inspect_data,
        section='containers')


@app.route('/containers/pause/<container>')
def pause(container):
    ...
    return render_template(
        "containers/after_commands/after_paused.html",
...
@app.route('/containers/unpause/<container>')
def unpause(container):
    config = docker_client().inspect_container(container)
    unpause_cmd = docker_client().unpause(container)
    params = container_detail()
    return render_template(
        "containers/after_commands/after_unpaused.html",
...
@app.route('/containers/stop/<container>')
def stop(container):
    config = docker_client().inspect_container(container)
    stop_cmd = docker_client().stop(container)
    params = container_detail()
    return render_template(
        "containers/after_commands/after_stopped.html",
...
@app.route('/containers/restart/<container>')
def restart(container):
    config = docker_client().inspect_container(container)
    restart_cmd = docker_client().restart(container, timeout=10)
    params = container_detail()
    return render_template(
        "containers/after_commands/after_restarted.html",
...
@app.route('/containers/remove/<container>')
def remove(container):
    ...
    try:
        ...
        return render_template(
            "containers/after_commands/after_removed.html",
...
@app.route('/containers/running')
def running():
    params = container_detail()

    return render_template(
        "containers/all_containers/running_all.html",
        running=params[2],
        section='containers'
    )


@app.route('/containers/paused')
def paused():
    params = container_detail()

    return render_template(
        "containers/all_containers/paused_all.html",
        paused=params[0],
        section='containers'
    )


@app.route('/containers/removed')
def removed():
    # params = container_detail()
    msg = "msg"
    rows = ""
    try:
        ...
        con.rollback()
        print(e)
        msg = "We can not add the removed container to the database. Error: \n" + e
    finally:
        return render_template(
            "containers/all_containers/removed_all.html",
            removed_containers=removed_containers,
            msg=msg,
            database=rows,
            section='containers'
...
@app.route('/containers/stopped')
def stopped():
    params = container_detail()

    return render_template(
        "containers/all_containers/stopped_all.html",
        stopped=params[4],
        section='containers'
    )
